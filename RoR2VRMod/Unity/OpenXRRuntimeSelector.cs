...
namespace VRMod.Unity
{
    internal class OpenXRRuntimeSelector
    {
        ...
        internal class SystemDefault : RuntimeDetector
        {
            ...
            public override string tooltip => RegistryHelper.GetValue(@"HKEY_LOCAL_MACHINE\SOFTWARE\Khronos\OpenXR\1", "ActiveRuntime", "");
        ...
        internal class OculusDetector : RuntimeDetector
        {
            ...
            public override string jsonPath
            {
                get
                {
                    var oculusPath = RegistryHelper.GetValue(installLocKey, installLocValue, "");
        ...
        {
            using var availableRuntimesKey = RegistryHelper.OpenSubKey(k_availableRuntimesRegistryKey);
            
            var runtimePathToValue = new Dictionary<string, int>();

            if (availableRuntimesKey == null) return GenerateRuntimeDetectorList(runtimePathToValue);

            var values = availableRuntimesKey.GetValues();
            foreach (var pair in values)
            {
                if (pair.Value is int intValue)
                {
                    runtimePathToValue.Add(pair.Key, intValue);
                }
            }
